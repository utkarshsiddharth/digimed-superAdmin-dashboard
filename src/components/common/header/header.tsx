...
const Header = ({ ThemeChanger }: any) => {
  ...
  //   if (theme.reducer.dataThemeMode != "dark") {
  //     ThemeChanger({
  //       ...theme.reducer,
  //       bodyBg1: "",
  //       bodyBg2: "",
  ...
  function menuClose() {
    const theme = store.getState();
    ThemeChanger({ ...theme.reducer, toggled: "close" });
  }

  const toggleSidebar = () => {
    const theme = store.getState();
    const sidemenuType = theme.reducer.dataNavLayout;
    if (window.innerWidth >= 992) {
      if (sidemenuType === "vertical") {
        const verticalStyle = theme.reducer.dataVerticalStyle;
        const navStyle = theme.reducer.dataNavStyle;
        switch (verticalStyle) {
          // closed
          case "closed":
            ThemeChanger({ ...theme.reducer, dataNavStyle: "" });
            if (theme.reducer.toggled === "close-menu-close") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "close-menu-close" });
            }
            break;
          // icon-overlay
          case "overlay":
            ThemeChanger({ ...theme.reducer, dataNavStyle: "" });
            if (theme.reducer.toggled === "icon-overlay-close") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              if (window.innerWidth >= 992) {
                ThemeChanger({
          ...
          case "icontext":
            ThemeChanger({ ...theme.reducer, dataNavStyle: "" });
            if (theme.reducer.toggled === "icon-text-close") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "icon-text-close" });
            }
            break;
          // doublemenu
          case "doublemenu":
            ThemeChanger({ ...theme.reducer, dataNavStyle: "" });
            if (theme.reducer.toggled === "double-menu-open") {
              ThemeChanger({ ...theme.reducer, toggled: "double-menu-close" });
            } else {
              const sidemenu = document.querySelector(
                ".side-menu__item.active"
              );
              if (sidemenu) {
                ThemeChanger({ ...theme.reducer, toggled: "double-menu-open" });
          ...
          // detached
          case "detached":
            if (theme.reducer.toggled === "detached-close") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "detached-close" });
            }
            break;
          // default
          case "default":
        ...
        switch (navStyle) {
          case "menu-click":
            if (theme.reducer.toggled === "menu-click-closed") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "menu-click-closed" });
            }
            break;
          // icon-overlay
          case "menu-hover":
            if (theme.reducer.toggled === "menu-hover-closed") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "menu-hover-closed" });
            }
            break;
          case "icon-click":
            if (theme.reducer.toggled === "icon-click-closed") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "icon-click-closed" });
            }
            break;
          case "icon-hover":
            if (theme.reducer.toggled === "icon-hover-closed") {
              ThemeChanger({ ...theme.reducer, toggled: "" });
            } else {
              ThemeChanger({ ...theme.reducer, toggled: "icon-hover-closed" });
            }
            break;
        }
      }
    } else {
      if (theme.reducer.toggled === "close") {
        ThemeChanger({ ...theme.reducer, toggled: "open" });

        setTimeout(() => {
          if (theme.reducer.toggled == "open") {
