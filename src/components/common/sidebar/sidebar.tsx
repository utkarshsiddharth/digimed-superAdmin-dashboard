...
import { MENUITEMS, MENUITEMSKOISE } from "./sidemenu/sidemenu";

import logo8 from "../../../assets/images/faces/logo8.png";
// import kiosk from "../../../assets/images/faces/kiosk.jpeg";
...
const Sidebar = ({ local_varaiable, ThemeChanger }: any) => {
  ...
  // const menuList = role === "koise" ? MENUITEMSKOISE : MENUITEMS;
  const menuList = role === "doctor" ? MENUITEMS : MENUITEMSKOISE;
  // const menuList = role === "doctor" ? MENUITEMSKOISE : MENUITEMS
  ...
  function Onhover() {
    ...
    if (
      (theme.reducer.toggled == "icon-overlay-close" ||
        theme.reducer.toggled == "detached-close") &&
      theme.reducer.iconOverlay != "open"
    ) {
      ThemeChanger({ ...theme.reducer, iconOverlay: "open" });
    }
  }
  function Outhover() {
    const theme = store.getState();
    if (
      ...
      ThemeChanger({ ...theme.reducer, iconOverlay: "" });
    }
  }
            ...
            if (items.children) {
              items.children.filter((submenu: any) => {
                ...
                if (dd === submenu.path + "/") {
                  const theme = store.getState();
                  items.active =
                    theme.reducer.dataNavLayout == "horizontal" ||
                    theme.reducer.dataNavStyle == "icon-hover"
                      ? false
                      : true;
                  items.selected = true;
                  submenu.active = true;
  ...
  const MenuOpen = (_event: any) => {
    ...
    if (
      ...
      const theme = store.getState();
      ThemeChanger({ ...theme.reducer, iconText: "open" });

      MainContent?.addEventListener("click", (_event) => {
        const theme = store.getState();
        ThemeChanger({ ...theme.reducer, iconText: "" });
      });
    }
    ...
    if (
      localStorage.ynexverticalstyles === "doublemenu" &&
      theme.reducer.toggled !== "double-menu-open"
    ) {
  ...
  return (
    <Fragment>
      ...
      >
        <div className="main-sidebar-header me-2">
          <Link
            to={`${import.meta.env.BASE_URL}superAdmin/user`}
